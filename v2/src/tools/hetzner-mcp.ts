/**
 * Hetzner MCP Tool Installer
 * Installs @ebowwa/hetzner-mcp - Hetzner VPS management MCP server
 *
 * Provides tools for:
 * - Server provisioning (create, delete, list, get info)
 * - Server operations (start, stop, reboot, rebuild)
 * - SSH key management
 * - Server onboarding
 */
...
export class HetznerMCPTool extends BaseTool {
  name = "hetzner-mcp";
  description = "@ebowwa/hetzner-mcp - Hetzner VPS management (servers, SSH keys, provisioning)";

  readonly NPM_PACKAGE = "@ebowwa/hetzner-mcp";

  async isApplicable(env: Environment): Promise<boolean> {
    // Hetzner MCP is useful for all environments except CI/CD
    return env.type !== "ci";
  }
  ...
  async install(env: Environment): Promise<void> {
    ...
    try {
      ...
      console.log(`  → MCP server: hetzner-mcp`);
    } catch {
