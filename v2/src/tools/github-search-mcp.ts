/**
 * GitHub Search MCP Tool Installer
 * Installs @ebowwa/github-search-mcp - GitHub search capabilities MCP server
 *
 * Provides tools for:
 * - Repository search
 * - Code search
...
export class GitHubSearchMCPTool extends BaseTool {
  name = "github-search-mcp";
  description = "@ebowwa/github-search-mcp - GitHub repository and code search";

  readonly NPM_PACKAGE = "@ebowwa/github-search-mcp";

  async isApplicable(env: Environment): Promise<boolean> {
    // GitHub Search MCP is useful for development environments
    return env.type !== "ci";
  }
  ...
  async install(env: Environment): Promise<void> {
    ...
    try {
      ...
      console.log(`  → MCP server: github-search-mcp`);
    } catch {
