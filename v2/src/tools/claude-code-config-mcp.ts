/**
 * Claude Code Config MCP Tool Installer
 * Installs @ebowwa/claude-code-config-mcp - Claude Code configuration management MCP server
 *
 * Provides tools for:
 * - Managing CLAUDE.md files
 * - Managing keybindings.json
...
export class ClaudeCodeConfigMCPTool extends BaseTool {
  name = "claude-code-config-mcp";
  description = "@ebowwa/claude-code-config-mcp - Claude Code config management (CLAUDE.md, settings, keybindings)";

  readonly NPM_PACKAGE = "@ebowwa/claude-code-config-mcp";

  async isApplicable(env: Environment): Promise<boolean> {
    // Claude Code Config MCP is useful for development environments
    return env.type !== "ci";
  }
  ...
  async install(env: Environment): Promise<void> {
    ...
    try {
      ...
      console.log(`  → MCP server: mcp-claude-code-config`);
    } catch {
