/**
 * Google Sheets MCP Tool Installer
 * Installs @mcp/googlesheets - Google Sheets integration MCP server
 *
 * Provides tools for:
 * - Managing Google Sheets spreadsheets
...
export class GoogleSheetsMCPTool extends BaseTool {
  name = "googlesheets-mcp";
  description = "@mcp/googlesheets - Google Sheets integration and management";

  readonly NPM_PACKAGE = "@mcp/googlesheets";

  async isApplicable(env: Environment): Promise<boolean> {
    // Google Sheets MCP is useful for development environments
    return env.type !== "ci";
  }
  ...
  async install(env: Environment): Promise<void> {
    ...
    try {
      ...
      console.log(`  → MCP server: mcp-googlesheets`);
    } catch {
