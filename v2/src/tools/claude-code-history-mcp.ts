/**
 * Claude Code History MCP Tool Installer
 * Installs @ebowwa/claude-code-history-mcp - Claude Code conversation history MCP server
 *
 * Provides tools for:
 * - Managing Claude Code conversation history
...
export class ClaudeCodeHistoryMCPTool extends BaseTool {
  name = "claude-code-history-mcp";
  description = "@ebowwa/claude-code-history-mcp - Claude Code conversation history with doppler integration";

  readonly NPM_PACKAGE = "@ebowwa/claude-code-history-mcp";

  async isApplicable(env: Environment): Promise<boolean> {
    // Claude Code History MCP is useful for development environments
    return env.type !== "ci";
  }
  ...
  async install(env: Environment): Promise<void> {
    ...
    try {
      ...
      console.log(`  → MCP server: claude-code-history-mcp`);
    } catch {
