#!/bin/bash
# ============================================================================
# Ralph Bash - Multi-Agent Version
# Supports multiple concurrent Ralph loops with separate state directories
# ============================================================================
...
# Parse --agent-id option first (before state dir is set)
...
STATE_FILE="${RALPH_DIR}/${AGENT_ID}-loop.txt"
LOG_DIR="${RALPH_DIR}/logs"
HISTORY_DIR="${RALPH_DIR}/history"
CURRENT_LOG="${LOG_DIR}/${AGENT_ID}.log"
...
read_state() {
  if [[ ! -f "$STATE_FILE" ]]; then
    echo "ERROR: No active loop found for agent '$AGENT_ID'"
    exit 1
  fi
...
archive_loop() {
  ...
  local archive_file="${HISTORY_DIR}/${AGENT_ID}-loop-${timestamp}-${status}.txt"

  cp "$STATE_FILE" "$archive_file"
  cp "$CURRENT_LOG" "${LOG_DIR}/${AGENT_ID}-loop-${timestamp}-${status}.log"
...
cmd_status() {
  if [[ ! -f "$STATE_FILE" ]]; then
    echo "=== Ralph Loop Status [$AGENT_ID] ==="
    echo ""
    echo "No active loop found for agent '$AGENT_ID'."
    exit 0
  fi
...
cmd_cancel() {
  if [[ ! -f "$STATE_FILE" ]]; then
    echo "No active loop to cancel for agent '$AGENT_ID'."
    exit 0
  fi

  source "$STATE_FILE"
  echo "Cancelling Ralph loop [$AGENT_ID] (iteration ${ITERATION})..."
...
cmd_continue() {
  if [[ ! -f "$STATE_FILE" ]]; then
    echo "No active loop to continue for agent '$AGENT_ID'."
    exit 0
  fi
  ...
  echo "=== Ralph Loop: Continuing [$AGENT_ID] ==="
  echo "Iteration: $ITERATION / $MAX_ITERATIONS"
  echo ""
...
You are agent [$AGENT_ID] running in a Ralph Loop.
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      --agent-id)
        AGENT_ID="$2"
        shift 2
        # Update state file paths with new agent ID
        STATE_FILE="${RALPH_DIR}/${AGENT_ID}-loop.txt"
        CURRENT_LOG="${LOG_DIR}/${AGENT_ID}.log"
        ;;
      --status)
  ...
  if [[ -f "$STATE_FILE" ]]; then
    echo "A Ralph loop is already running for agent '$AGENT_ID'!"
    echo ""
    echo "Options:"
    echo "  ./ralph.sh --agent-id $AGENT_ID --status    # See what's running"
    echo "  ./ralph.sh --agent-id $AGENT_ID --continue  # Resume the loop"
    echo "  ./ralph.sh --agent-id $AGENT_ID --cancel    # Cancel it first"
    exit 1
  fi
  ...
  if [[ -z "$prompt" ]]; then
    ...
    echo "Usage: ./ralph.sh --agent-id <id> \"Your task here\""
    exit 1
  fi

  # Create new loop
  echo "=== Starting Ralph Loop [$AGENT_ID] ==="
  echo "Prompt: $prompt"
